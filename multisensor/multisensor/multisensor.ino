...
bool processJson(char* message) {
  ...
  if (jsonDoc.containsKey("flash")) {
    flashLength = (int)jsonDoc["flash"] * 1000;

    if (jsonDoc.containsKey("brightness")) {
      flashBrightness = jsonDoc["brightness"];
    }
    else {
    ...
    if (jsonDoc.containsKey("color")) {
      flashRed = jsonDoc["color"]["r"];
      flashGreen = jsonDoc["color"]["g"];
      flashBlue = jsonDoc["color"]["b"];
    }
    else {
      flashRed = red;
      flashGreen = green;
...
//  JsonObject root = jsonDoc.to<JsonObject>();
  ...
  jsonDoc["humidity"] = (String)humValue;
  jsonDoc["motion"] = (String)motionStatus;
  jsonDoc["ldr"] = (String)LDR;
